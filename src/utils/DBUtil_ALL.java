...
public class DBUtil_ALL {
	...
	public static Session getSess() {
		...
		op.setConsistencyLevel(ConsistencyLevel.ALL);
		LoadBalancingPolicy loadBalancingPolicy = new RoundRobinPolicy();
		cluster = Cluster.builder().addContactPoint("159.226.41.232").withPort(10056).withPoolingOptions(poolingOptions)
				.withSocketOptions(options).withQueryOptions(op).build();
